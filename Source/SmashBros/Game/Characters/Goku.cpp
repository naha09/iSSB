...
namespace SmashBros
{
	void Goku::addProjectileInfo(byte type, int projID, float x, float y)
	{
		ProjectileInfo info;
		info.type = type;
		info.projID = projID;
	...
	Goku::Goku(float x1, float y1, byte playerNo, byte team) : Player(x1, y1, playerNo, team)
	{
		turning = false;
		winding = false;
		finalSmashing = false;
	...
	void Goku::addP2PData(DataVoid&data)
	{
	...
	void Goku::setP2PData(byte*&data)
	{
	...
	void Goku::Load()
	{
		setFolderPath("Images/Game/Characters/Goku/");
		
		loadFile("Images/Game/Characters/Goku/spring_normal.png");
		loadFile("Images/Game/Characters/Goku/spring_bounce.png");
		loadFile("Images/Game/Characters/Goku/finalsmash_emeralds.png");
		addTwoSidedAnimation("stand", "stand.png", 4, 7, 1);
		...
		anim->addFrame("Images/Game/Characters/Goku/finalsmash_turn.png");
		addAnimation(anim);
	...
	void Goku::Update(long gameTime)
	{
		        ...
		        else
		        {
					...
					if(moving)
					{
						...
						else if(animName.equals((String)"finalsmash_turn" + suffix))
						{
							...
							if(animpos<=0)
							{
								x += gokuSpeedRun*0.25f * getPlayerDirMult();
							}
							else if(animpos<=1)
							{
								x += gokuSpeedRun*0.5f * getPlayerDirMult();
							}
							else
							{
								x += gokuSpeedRun*0.75f * getPlayerDirMult();
							}
						}
						else
						{
							x += gokuSpeedRun * getPlayerDirMult();
	...
	void Goku::Draw(Graphics2D&g, long gameTime)
	{
		Player::Draw(g, gameTime);
	}
	
	void Goku::onAnimationFinish(const String&n)
	{
	...
	void Goku::onPlayerHit(Player*collide, byte dir)
	{
		switch(dir)
		{
			case DIR_LEFT:
	...
	void Goku::onPlayerRectHit(Player*collide, byte dir)
	{
		switch(dir)
		{
			case DIR_LEFT:
	...
	void Goku::onFinishCharge()
	{
		//
	}
	
	void Goku::doChargingAttack(byte button)
	{
		if(chargeAttack==1)
        {
            destroyCharge();
	...
	void Goku::attackSideSmash(byte type)
	{
	...
	void Goku::attackUpSmash(byte type)
	{
	...
	void Goku::attackDownSmash(byte type)
	{
	...
	Goku::Spring::Spring(byte playerNo, float x1, float y1) : Projectile(playerNo,x1,y1)
	{
		deadTime=Global::getWorldTime()+5000;
			
		addAnimation(new Animation("normal", 1, "Images/Game/Characters/Goku/spring_normal.png"));
		Animation*anim = new Animation("bounce", 20, 5, 1);
		anim->addFrame("Images/Game/Characters/Goku/spring_bounce.png");
		addAnimation(anim);
	...
	void Goku::Spring::Update(long gameTime)
	{
		yvelocity+=0.5;
	...
	void Goku::Spring::onAnimationFinish(const String&n)
	{
		if(n.equals("bounce"))
		{
			changeAnimation("normal",FORWARD);
	...
	void Goku::Spring::onPlayerHit(Player*collide, byte dir)
	{
		if(dir==DIR_UP)
		{
	...
	void Goku::Spring::whilePlatformColliding(Platform*collide, byte dir)
	{
		switch(dir)
		{
			case DIR_UP:
	...
	Goku::Emeralds::Emeralds(byte playerNo, float x1, float y1) : Projectile(playerNo, x1, y1)
	{
		Animation*anim = new Animation("appear", 10, 7, 1);
		anim->addFrame("Images/Game/Characters/Goku/finalsmash_emeralds.png");
		addAnimation(anim);
	...
	Goku::Emeralds::~Emeralds()
	{
		//
	}
	
	void Goku::Emeralds::onAnimationFinish(const String&n)
	{
		destroy();
	}
}