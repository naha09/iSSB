...
namespace SmashBros
{
	void Zero::addProjectileInfo(byte type, int projID, float x, float y)
	{
		ProjectileInfo info;
		info.type = type;
		info.projID = projID;
	...
	Zero::Zero(float x1, float y1, byte playerNo, byte team) : Player(x1, y1, playerNo, team)
	{
		charging = 0;
		finalSmash = false;
	...
	void Zero::addP2PData(DataVoid&data)
	{
	...
	void Zero::setP2PData(byte*&data)
	{
	...
	void Zero::Load()
	{
		setFolderPath("Images/Game/Characters/Zero/");
		
		loadFile("Images/Game/Characters/Zero/projectile.png");
		loadFile("Images/Game/Characters/Zero/down_blast.png");
		loadFile("Images/Game/Characters/Zero/finalsmash_blast.png");
		loadFile("Images/Game/Characters/Zero/blade.png");
	...
	void Zero::onAnimationFinish(const String&n)
	{
	...
	void Zero::Update(long gameTime)
	{
	...
	void Zero::Draw(Graphics2D&g, long gameTime)
	{
	...
	void Zero::whilePlayerColliding(Player*collide, byte dir)
	{
		if(attacksHolder==11)
		{
	...
	void Zero::onPlayerHit(Player*collide, byte dir)
	{
		switch(dir)
		{
			case DIR_LEFT:
	...
	void Zero::onFinishCharge()
	{
		//
	}
	
	void Zero::doChargingAttack(byte button)
	{
		if(charging==1)
		{
			destroyCharge();
	...
	void Zero::attackSideSmash(byte type)
	{
	...
	void Zero::attackUpSmash(byte type)
	{
	...
	void Zero::attackDownSmash(byte type)
	{
	...
	Zero::Blast::Blast(byte playerNo, float x1, float y1) : Projectile(playerNo, x1, y1)
	{
		Animation*anim;
		anim = new Animation("left",1,"Images/Game/Characters/Zero/projectile.png");
		addAnimation(anim);
			
		anim = new Animation("right",1,"Images/Game/Characters/Zero/projectile.png");
		anim->mirror(true);
		addAnimation(anim);
	...
	void Zero::Blast::deflect(byte dir)
	{
		switch(dir)
		{
			case DIR_LEFT:
	...
	void Zero::Blast::Update(long gameTime)
	{
	...
	void Zero::Blast::onPlayerHit(Player*collide, byte dir)
	{
	...
	Zero::SpinningBlade::SpinningBlade(byte playerNo, float x1, float y1) : Projectile(playerNo,x1,y1)
	{
		Animation*anim;
			
		anim = new Animation("left",12,8,1);
		anim->addFrame("Images/Game/Characters/Zero/blade.png");
		addAnimation(anim);
	...
	Zero::SpinningBlade::~SpinningBlade()
	{
		//
	};
	
	void Zero::SpinningBlade::Update(long gameTime)
	{
	...
	void Zero::SpinningBlade::Draw(Graphics2D&g, long gameTime)
	{
		Player*owner = Global::getPlayerActor(getPlayerNo());
	...
	void Zero::SpinningBlade::onAnimationFinish(const String&n)
	{
		Player*owner = Global::getPlayerActor(getPlayerNo());
	...
	void Zero::SpinningBlade::onPlayerHit(Player*collide, byte dir)
	{
		if(isHittable(collide,dir))
		{
	...
	Zero::GroundStab::GroundStab(byte playerNo, float x1, float y1) : Projectile(playerNo, x1,y1)
	{
		Animation*anim;
		anim = new Animation("normal",6,4,1);
		anim->addFrame("Images/Game/Characters/Zero/down_blast.png");
		addAnimation(anim);
	...
	Zero::GroundStab::~GroundStab()
	{
		//
	}
		
	void Zero::GroundStab::onPlayerHit(Player*collide, byte dir)
	{
	...
	void Zero::GroundStab::onAnimationFinish(const String&n)
	{
		if(n.equals("normal"))
		{
			destroy();
	...
	Zero::FinalSmashBlast::FinalSmashBlast(byte playerNo, float x1, float y1) : Projectile(playerNo, x1,y1)
	{
		setScale(1.6f);
		
		addAnimation(new Animation("left",1,"Images/Game/Characters/Zero/finalsmash_blast.png"));
		Animation*anim = new Animation("right",1,"Images/Game/Characters/Zero/finalsmash_blast.png");
		anim->mirror(true);
		addAnimation(anim);
	...
	Zero::FinalSmashBlast::~FinalSmashBlast()
	{
		//
	}
		
	void Zero::FinalSmashBlast::onPlayerHit(Player*collide, byte dir)
	{
		if(isHittable(collide,dir))
		{
			switch(dir)
